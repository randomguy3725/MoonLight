                    ...
                    switch (grimMode.get()) {
                        case "Reduce":
                            if (getModule(KillAura.class).target != null && !isEnabled(Scaffold.class)) {
                                ...
                                double velocityX = s12.getMotionX() / 6000.0;
                                double velocityZ = s12.getMotionZ() / 6000.0;

                                if (MathHelper.sqrt_double(velocityX * velocityX * velocityZ * velocityZ) <= 3F) {
                                    mc.thePlayer.motionX = mc.thePlayer.motionZ = 0.15;
                                } else {
                                    mc.thePlayer.motionX = velocityX * 0.1;
                                    mc.thePlayer.motionZ = velocityZ * 0.1;
                                }
